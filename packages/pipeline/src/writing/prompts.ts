...
- metadata: summary only — a short 1-2 sentence teaser for the home page, calendar, and SEO, distinct from day_overview and never shown inside the brief itself. It must describe only the items you actually returned in developments and worth_watching, never a story you reviewed and left out.

Do not return counts, intensity, company/topic lists, development digests, or source counts in metadata. Code derives those fields deterministically from the final returned items, their referenced research stories, and the citations actually shown.`;
